    ...
    <div class="container mx-auto px-6 py-12">
      ...
      <div class="flex flex-col md:flex-row items-start justify-center gap-12">
        ...
        <div class="relative w-full md:w-1/2 mt-4">
          <Globe ref="globeRef" @ready="onGlobeReady" />
        </div>
        ...
        <form @submit.prevent="submitForm" class="max-w-lg w-full md:w-2/3 mx-auto space-y-6 mt-8">
          ...
          <div>
            ...
            <input v-model="form.email" id="email" type="email" placeholder="example@domain.com"
              class="w-full p-4 bg-black text-white rounded-lg border border-gray-700 focus:outline-none focus:ring-2 focus:ring-accent" />
            <p v-if="errors.email" class="text-red-500 text-sm mt-1">Bitte geben Sie eine gültige E-Mail-Adresse ein.
            </p>
          </div>

          <div>
            <label for="message" class="block text-sm text-gray-400 mb-2">Ihre Nachricht</label>
            <textarea v-model="form.message" id="message" rows="5" placeholder="Ihre Nachricht..."
              class="w-full p-4 bg-black text-white rounded-lg border border-gray-700 focus:outline-none focus:ring-2 focus:ring-accent"></textarea>
            <p v-if="errors.message" class="text-red-500 text-sm mt-1">Bitte geben Sie eine Nachricht ein.</p>
          </div>

          <button type="submit" class="w-full py-3 px-6 rounded-full border-2 border-gray-500 text-white transition-all shadow-lg
